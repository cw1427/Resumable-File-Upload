  ...
  var Resumable = function(opts){
    ...
    var $h = {
      ...
      generateUniqueIdentifier:function(file){
        ...
        if(typeof custom === 'function') {
          //return custom(file);
          //----changed by cw use promise to do a sync call for async
          return new Promise(function(resolve, reject) {
              custom(file).then(function(value){
                  resolve(value);
              });
          });
        }else {
            //----check if has the resumable extension for the genMd5 function
            if ($.genMd5) {
                return new Promise(function(resolve, reject) {
                    $.genMd5(file).then(function(value){
                        resolve(value);
                    });
                });
            }else {
                return new Promise(function(resolve, reject) {
                  ...
                  var size = file.size;
                  var identify = size + '-' + relativePath.replace(/[^0-9a-zA-Z_-]/img, '');
                  resolve(identify);
                });
            }
        }
    ...
    var appendFilesFromFileList = function(fileList, event){
      ...
      $h.each(fileList, function(file){
        ...
        //----changed by cw use promise to set the call process by sync
        //var uniqueIdentifier = $h.generateUniqueIdentifier(file)
        var uniqueIdentifier = null;
        $h.generateUniqueIdentifier(file).then(function(value){
    ...
    function ResumableFile(resumableObj, file, uniqueIdentifier){
      ...
      $.cancel = function(){
        //----changed by cw check if file is completed then ignore cancel event
        if ($.isComplete()) {
            return;
        }
      ...
      $.bootstrap = function(){
        ...
        for (var offset=0; offset<maxOffset; offset++) {(function(offset){
            window.setTimeout(function(){
                //----changed by cw
                var tempResumableChunk = new ResumableChunk($.resumableObj, $, offset, chunkEvent);
                $.chunks.push(tempResumableChunk);
      ...
      //---changed by cw added the init functio for the resumableFile
      $.initSet = function() {
          var len = $.chunks.length;
          if ($.chunkStartOffset > len){
              //console.log('----inner error chunk offset should not large then the chunks total size');
              throw new Error('chunk start offset is not correct');
          }
          for (var offset=0; offset<$.chunkStartOffset; offset++){
              $.chunks[offset].tested = true;
              $.chunks[offset].ignore = true;
              //console.log('----set current chunk tested=true ignore=true');
          }
      }
      //---added by cw added file event
      $.events = []; //---pause cancel upload
      $.on = function(event,callback){
        $.events.push(event.toLowerCase(), callback);
      };
      $.fire = function(){
    ...
    function ResumableChunk(resumableObj, fileObj, offset, callback){
      ...
      $.ignore = false;//----added by cw for ignore testing this chunk
      ...
      $.send = function(){
        ...
        if($.getOpt('testChunks') && !$.tested) {
          $.test();
          return;
        }else{//----added by cw for ignore
            if ($.ignore) { //----ignore this chunk test and go to next chunk
                $.preprocessState = 2;
      ...
      $.status = function(){
         //----changed by cw pretend for ignore chunk's xhr is success
        if ($.ignore) {
            return('success');
        }
      ...
      $h.each($.files, function(file){
        if(!file.isComplete()) {
          ...
          return(false);
            ...
            //----changed by cw for completed file also need to trigger fileSuccess event
            file.resumableObj.fire('fileSuccess', file, null);
        }
      });
      if(!outstanding) {
    ...
    //----changed by cw add file event entrance
    $.triggerFileEvent = function(identifier,event){
        console.log('----trigger file event occured ' + identifier + '---event=' + event);
        $h.each($.files, function(file){
            if (identifier === file.uniqueIdentifier){
                console.log('----found file='+identifier+' start fire event=' + event);
                file.fire(event);
                return;
            }
        });
